                ...
                <div className='pt-2 space-y-2'>
                    {
                        categories.map((item, key) => (
                            <Link href={`/shop/category/${item.id}`} key={key} className="inline-flex justify-between items-center rounded-md border border-gray-200 w-full p-1.5 space-x-1.5">
                ...
                </div>
                {
                    !isFetching && categories.length > 0 ? (
                        ...
                        >
                            {categories.map((item, key) => (
                                <li key={key}>
                                    <Link href={`/shop/category/${item.id}`} className="justify-between capitalize">
                                        { item.name }
                                    </Link>
                                </li>
                            ))}
